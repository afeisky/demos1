...
public class Down {
    ...
    private cURLs[] listUrls=new cURLs[LISTURLS_LEN];
    ...
    public void start(int cmd) {
        ...
        if (cmd==0) {
            ...
            if (!(time1 >= 1815 || time1 <= 800) || isFinish) {
                timerCount++;
                show_callback("Timer:"+timeNow,timerCount%100);
                startTimer(300000);
                return;
            }else{
                isFinish=false;
...
*/
        try {
            ...
            for (int i=0;i<listUrls.length;i++){
                ...
                Logd(i+":"+u.type+u.url1+u.url2+u.url3);
            }
                ...
                if (u.type.equals("sn")){
                    ...
                    if (data.indexOf("{\"code\":") == 0) {
                        ...
                        if (jsonItems.length() == 0) {
                            ...
                            Loge( "json_count:" + "[" + u.jsono.getInt("n") + "/" + u.jsono.getInt("count"));
                            Loge( "json_count:" + "[" + json_n + "/" + u.jsono.getInt("count"));
                            ...
                            if (u.jsono.getString("day").length() == 0) {
                                ...
                                if (!u.jsono.getString("day").equals(json.getString("day"))) {
                                    Loge( "Error: " + u.jsono.getString("day") + " =?" + json.getString("day"));
                                }
                            }
    ...
    private boolean zipFile(){
        ...
        if (!fzip.exists()){
            ...
            for (int i=0;i<LISTURLS_LEN;i++){
                ...
                if (u==null)
                    break;
                Loge(i+":"+u.filePathName);
                files.add(new File(u.filePathName));
            }
            try {
                Loge("zipfile:"+fzip.getAbsolutePath());
