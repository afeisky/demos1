...
public class DbContentProvider extends ContentProvider {
    ...
    public Cursor queryBK1(String codeKey,String date,int type){
        ...
        String selection=TBK.COLUMN_TIME+"=? and substr("+TBK.COLUMN_CODE+",0,?)==?";//String selection="date=\"?\" and substr(code,0,?)==\"?\"";
    ...
    public boolean queryBK11(String codeKey, String date, int type, List<Map<String,Object>> datalist){
        ...
        String table="snbk";
        //String[] colums=new String[]{"code","name","number","count","volume","amount","trade","symbol","sname","strade","scgpercent"};
        String[] colums=new String[]{TBK.COLUMN_TIME,TBK.COLUMN_CODE,TBK.COLUMN_NAME,
                TBK.COLUMN_NUMBER,TBK.COLUMN_COUNT,
                TBK.COLUMN_VOLUME,TBK.COLUMN_AMOUNT,
                TBK.COLUMN_TRADE,TBK.COLUMN_CHANGEPERCENT,
                TBK.COLUMN_SYMBOL,TBK.COLUMN_SNAME,
                TBK.COLUMN_STRADE,TBK.COLUMN_SCHANGEPERCENT};
        String selection=TBK.COLUMN_TIME+" in (select "+TBK.COLUMN_TIME+" from "+table+" order by "+TBK.COLUMN_TIME+" DESC limit 1) and substr("+TBK.COLUMN_CODE+",0,?)==?";//String selection="date=\"?\" and substr(code,0,?)==\"?\"";
        ...
        try {
            ...
            if (c != null) {
                LogX.w(TAG, "333");
                for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
                    int COLUMN_DATE = c.getColumnIndex(TBK.COLUMN_TIME);
                    int COLUMN_CODE = c.getColumnIndex(TBK.COLUMN_CODE);
                    int COLUMN_NAME = c.getColumnIndex(TBK.COLUMN_NAME);
                    int COLUMN_NUMBER = c.getColumnIndex(TBK.COLUMN_NUMBER);
                    int COLUMN_COUNT = c.getColumnIndex(TBK.COLUMN_COUNT);
    ...
    public JSONObject queryBK12(String codeKey, String date, int type){
        ...
        int len=codeKey.length()+1;
        String table="snbk";
        String sql1="select * "//+TBK.COLUMN_TIME+","+TBK.COLUMN_CODE+","+TBK.COLUMN_NAME
                +" from "+table+" "
                +" where substr("+TBK.COLUMN_CODE+",0,"+len+")=\""+codeKey+"\" and "+TBK.COLUMN_TIME+" in (select "+TBK.COLUMN_TIME+" from "+table
                +" order by "+TBK.COLUMN_TIME+" DESC limit 1)"
                +"order by "+TBK.COLUMN_CHANGEPERCENT+" DESC";
        String sql="select * "//+TBK.COLUMN_TIME+","+TBK.COLUMN_CODE+","+TBK.COLUMN_NAME
                +" from "+table+" "
                +" where substr("+TBK.COLUMN_CODE+",0,?)=? and "+TBK.COLUMN_TIME+" in (select "+TBK.COLUMN_TIME+" from "+table
                +" order by "+TBK.COLUMN_TIME+" DESC limit 1)"
                +"order by "+TBK.COLUMN_CHANGEPERCENT+" DESC";
        //LogX.w(TAG, "[ "+sql);
        try {
    ...
    public JSONObject queryBK11(){
        ...
        String sql="select * "//+TBK.COLUMN_TIME+","+TBK.COLUMN_CODE+","+TBK.COLUMN_NAME
                +" from "+table+" "
                +" where substr("+TBK.COLUMN_CODE+",0,?)=? and abs("+TBK.COLUMN_CHANGEPERCENT+")>1 and "+TBK.COLUMN_TIME+" in (select "+TBK.COLUMN_TIME+" from "+table
                +" order by "+TBK.COLUMN_TIME+" DESC limit 1)"
                +"order by "+TBK.COLUMN_CHANGEPERCENT+" DESC ";
    ...
    public JSONObject queryBK10(){
        ...
        int len;
        String table="snbk";
        String sql="select * from ( select *,2 as z1,abs("+TBK.COLUMN_CHANGEPERCENT+")as z2 "//+TBK.COLUMN_TIME+","+TBK.COLUMN_CODE+","+TBK.COLUMN_NAME
                +" from "+table+" "
                +" where substr("+TBK.COLUMN_CODE+",0,?)=? and "+TBK.COLUMN_CHANGEPERCENT+">1 and "+TBK.COLUMN_TIME+" in (select "+TBK.COLUMN_TIME+" from "+table
                +" order by "+TBK.COLUMN_TIME+" DESC limit 1)"
                +"order by "+TBK.COLUMN_CHANGEPERCENT+" DESC )"
                +"union select * from ( select *,1 as z1,abs("+TBK.COLUMN_CHANGEPERCENT+")as z2 "//+TBK.COLUMN_TIME+","+TBK.COLUMN_CODE+","+TBK.COLUMN_NAME
                +" from "+table+" "
                +" where substr("+TBK.COLUMN_CODE+",0,?)=? and "+TBK.COLUMN_CHANGEPERCENT+"<-1 and "+TBK.COLUMN_TIME+" in (select "+TBK.COLUMN_TIME+" from "+table
                +" order by "+TBK.COLUMN_TIME+" DESC limit 1)"
    ...
    public JSONObject querySSE10(){
        ...
        String date = TSSE.COLUMN_TIME;//"date";
    ...
    public JSONObject queryBK111(){
        JSONObject json=new JSONObject();//"{\"time\":\"\",\"gn\":[],,\"hy\":[],,\"new\":[]}"
        JSONArray gn=new JSONArray();
        JSONArray hy=new JSONArray();
        ...
        try {
            ...
            for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
                ...
                one.put(0,c.getString(2).toString());
                one.put(1,Double.valueOf(c.getString(3).toString()));
                if (c.getString(1).toString().equals("gn")){
                    gn.put(gn.length(),one);
                }
                if (c.getString(1).toString().equals("hangye")){
                    hy.put(hy.length(),one);
                }
                if (c.getString(1).toString().equals("new")){
                    ne.put(ne.length(),one);
                }
                if (stime.length()==0) {
                    LogX.w(TAG,c.getString(0).toString());
                    stime =c.getString(0).toString(); // why "2018-04-12 15:14:33.0"
                    stime=stime.replace(".0",""); // del ".0"
                    LogX.e(TAG,stime);
                }
            ...
            LogX.w(TAG, json.toString().substring(0,json.toString().length()>100?100:0));
